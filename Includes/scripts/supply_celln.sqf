_veh = vehicle player;

// items and attachments go here
_veh addItemCargoGlobal ["ACE_elasticBandage",20];
_veh addItemCargoGlobal ["ACE_epinephrine",10];
_veh addItemCargoGlobal ["ACE_morphine",10];
_veh addItemCargoGlobal ["ACE_packingBandage",20];
_veh addItemCargoGlobal ["ACE_tourniquet",4];
_veh addItemCargoGlobal ["ACE_salineIV",8];
_veh addItemCargoGlobal ["ACE_Tripod",1];
_veh addItemCargoGlobal ["ToolKit",1];
_veh addItemCargoGlobal ["ACE_wirecutter",1];
_veh addItemCargoGlobal ["ACE_EarPlugs",2];
_veh addItemCargoGlobal ["ACE_EntrenchingTool",2];
_veh addItemCargoGlobal ["ACE_CableTie",4];
_veh addItemCargoGlobal ["ACE_DefusalKit",1];
_veh addItemCargoGlobal ["ACE_M26_Clacker",1];
_veh addItemCargoGlobal ["tf47_optic_m3maaws",1]; 
_veh addItemCargoGlobal ["ACE_NVG_Wide",2];
_veh addItemCargoGlobal ["ACE_IR_Strobe_Item",6];
_veh addItemCargoGlobal ["ACE_SpareBarrel",1];

// magazines and explosives go here
_veh addMagazineCargoGlobal ["hlc_20Rnd_762x51_B_fal",12];
_veh addMagazineCargoGlobal ["tf47_m3maaws_HEDP",4];
_veh addMagazineCargoGlobal ["tf47_m3maaws_HEAT",2];
_veh addMagazineCargoGlobal ["tf47_m3maaws_HE",4];
_veh addMagazineCargoGlobal ["tf47_m3maaws_ILLUM",4];
_veh addMagazineCargoGlobal ["tf47_m3maaws_HEDP",4];
_veh addMagazineCargoGlobal ["HandGrenade",4];
_veh addMagazineCargoGlobal ["B_IR_Grenade",6];
_veh addMagazineCargoGlobal ["ACE_Chemlight_HiYellow",10];
_veh addMagazineCargoGlobal ["ACE_HandFlare_Yellow",10];
_veh addMagazineCargoGlobal ["rhs_mag_mk84",2];
_veh addMagazineCargoGlobal ["DemoCharge_Remote_Mag",4];
_veh addMagazineCargoGlobal ["SLAMDirectionalMine_Wire_Mag",2];
_veh addMagazineCargoGlobal ["APERSTripMine_Wire_Mag",2];
_veh addMagazineCargoGlobal ["rhsgref_mag_rkg3em",1];
_veh addMagazineCargoGlobal ["ACE_HuntIR_M203",4];
_veh addMagazineCargoGlobal ["rhs_mag_m661_green",6];


// weapons go here 
_veh addWeaponCargoGlobal ["hlc_rifle_falosw",2];
_veh addWeaponCargoGlobal ["tf47_m3maaws",1];
_veh addWeaponCargoGlobal ["ACE_VMM3",1];
_veh addWeaponCargoGlobal ["rhs_weap_M320",1];

hint "Cell Supplies Night Ops Loaded";
